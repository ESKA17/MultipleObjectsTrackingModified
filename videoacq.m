...
stop(vid); flushdata(vid); delete(vid);

%% Image processing
function [bboxes, centroids, pts, linepts, myang, refbbox] = improc()
% Finding sticker
Isub = imsubtract(frame(:,:,1), rgb2gray(frame));
Isub = imgaussfilt(Isub, 2);
Isub = imbinarize(Isub);
% Isub = bwareaopen(Isub, 300);
regprops = regionprops(Isub, 'Orientation', 'BoundingBox');
if ~isempty(regprops)
